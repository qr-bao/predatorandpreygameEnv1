...
class LISPredatorPreyEnv(ParallelEnv):
    ...
    def step(self, actions):
        ...
        for agent in self.agents:
            if terminated[agent] or truncations[agent]:
                to_remove.append(agent)

        for agent in to_remove:
            self.agents.remove(agent)

        # print(self.agents,end="\t")
        # print(terminated)
    ...
    def _process_agents(self):
        ...
        new_state = {agent: live_agents[agent].get_observe_info() if agent in live_agents and live_agents[agent].is_alive == True else zero_data for agent in self.agents}
        rewards = {agent: self._compute_reward(live_agents[agent]) if agent in live_agents and live_agents[agent].is_alive == True else 0 for agent in self.agents}
        infos = {agent: {} for agent in self.agents}
        dones = {agent: False if agent in live_agents and live_agents[agent].is_alive == True else False for agent in self.agents}
        # new_state = {agent: agent.get_observe_info() for agent in self.agents}
        # for name in self.agents:
        #     matching_agent = next((agent for agent in self.simulator.preys + self.simulator.predators if agent.name == name), None)
        #     if matching_agent and matching_agent.is_alive:
        #         live_agents[name]=matching_agent
...
if __name__ == "__main__":
    ...
    env_name = "LISPredatorPreyEnv"
    def env_creator(env_config):
        # 创建并返回 PettingZoo 环境的实例
        return LISPredatorPreyEnv(
    ...
    print(env.totalobservation_space.contains(env.reset()[0]))
    # # ParallelPettingZooEnv(env_creator("LISPredatorPreyEnv"))
    # register_env(env_name,lambda config:ParallelPettingZooEnv(env_creator("LISPredatorPreyEnv")))
